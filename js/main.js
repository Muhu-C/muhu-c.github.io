...
const initializeApp = async () => {
  ...
  // auto 模式无法在渲染时确定实际模式，初始化时按当前 data-theme 更新右键菜单文案
  const darkModeText = document.querySelector(".menu-darkmode-text");
  if (darkModeText && Solitude.config.right_menu?.mode) {
    const isDark =
      document.documentElement.getAttribute("data-theme") === "dark";
    darkModeText.textContent = isDark
      ? Solitude.config.right_menu.mode.light
      : Solitude.config.right_menu.mode.dark;
  }
};
